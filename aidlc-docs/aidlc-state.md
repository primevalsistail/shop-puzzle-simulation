...
- **評価の宿題**: ユーザー判断は **A〜E を片づけてから Phase 4**（[質問票](construction/item-taxonomy/evaluation/phase3-followup-questions.md) Q1〜Q4 すべて A）
  ...
  - [ ] **C. 解禁条件 100 の作り直し**（スコープ**内**と決定）
        **前提は 2026-09-07 に確定した** → [質問票](construction/item-taxonomy/evaluation/material-unlock-questions.md)
        **素材（tier1）の産地依存は最後まで残す**（解禁を素材に広げない）。
        **宿題C は加工品の解禁だけを作り直す**
        **⚠ 併せて確認すること**: 加工品側の U2 に「投げ売りして解放だけ回収する」経路があるか
        （遊ぶ人5人のうち4人が素材版でこの穴を独立に指摘した。**加工品側は誰も見ていない**）
  - [ ] **D. R5・D2 の `産地` 直読みを直す**（加工品60品に1品も当たらない）
  - [ ] **E. rules.ts の U4 と evaluate.ts の食い違いを直す**（評価器が `r.id === 'U1'` とIDで名指し）
- **Next Stage**: **宿題C — 加工品の解禁条件（100）の作り直し**（終わったら D・E → Phase 4）
