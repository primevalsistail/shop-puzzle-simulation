...
⚠ **来店客は左上を向く**（キャラ帯は画面の右端で、売り場はその左）。**正面向きだと画面に合わない。**
**6通りは 男子・女子・男性・女性・老人男性・老人女性**（PO 指定 2026-09-14）。
⚠ **届いていた24体と、見本にしていた `sd/customer-style/` は消した**（PO 判断 2026-09-15）。
**前回の出力を見本に添えると、画風だけでなく向きも寄る**（2026-09-14 に踏んだ）。
**見本は `sd/baren_customer.png` ／ `sd/noela_customer.png` の2枚**——
**名前のある9人の原本で、前回の出力ではない。**24体は0から描き直す。
