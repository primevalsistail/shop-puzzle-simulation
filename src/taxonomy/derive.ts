 ...
 * 利益額の目盛り。
 *
 * ⚠ **5.5 → 13.5 に上げた**（段4 の置き直し 2026-09-15 ／ `stage4-rebalance-160-200.md`）。
 *   **これが到達日を決めるほぼ唯一のつまみ。**目標 2,000,000 への到達日（測る道具・種4つ）:
 *
 * | 遊び方 | 種7 | 種11 | 種3 | 種23 | 帯 160〜200 |
 * |---|---|---|---|---|---|
 ...
 *
 * ## ⚠ 水準しか動かせない —— 深い品だけを持ち上げるつまみは 層1 に無い
 *
 * `浅い品だけ作る` と `深い品を作る` の差は **35〜40日**で、**帯の幅とほぼ同じ。**
 * **縮めようとして3つとも空振りした**（2026-09-15 に実測）:
 *
 * | 試したこと | なぜ効かないか |
 * |---|---|
 * | **`CELL_EXPONENT` を上げる** | `深い品を作る` は **tier4 止まり**で、升は **3.28**。<br>`浅い品だけ作る` の tier2 は **2.05**。1.2→1.25 で差は **2%** しか開かない |
 * | **`LUXURY_PROFIT` の贅沢を上げる** | **格は深さと相関しない。**贅沢の割合は<br>**tier2 が 30%、tier3 が 19%** —— **むしろ浅いほうが贅沢。逆に効く** |
 * | **行商人の割増を下げる**（1.5→1.2） | **種によって上下する**（種7 は 203→234 と**悪化**、種11 は 218→203）。**戻した** |
 *
 * **だから両方を帯に入れるには、水準を上げて帯の中を滑らせるしかない。**
 * **13.5 はその窓の真ん中**（浅い品の下端と深い品の上端が同時に帯へ入る、ほぼ唯一の値）。
 *
