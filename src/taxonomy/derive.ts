 ...
 *
 * **1.1 はこの幅の下寄り**で、いちばん大きい品でも**升あたりの上乗せは `9^0.1 = 1.246倍`**。
 * ⚠ **1.262 を超えると「大きい品だけ」に一本化して、かたちの意味が消える**（経営シム好き）。
 */
const CELL_EXPONENT = 1.1

/**
 * 利益額の目盛り。
 *
 * ⚠ **底が変わったので測り直した**（分 15〜480 → 升 1〜9。2026-09-15）。
 *   **全106レシピの加工利益の中央値を、作り替える前（20.19）に合わせてある** ——
 *   こうしないと、**材料費（tier1 の `basePrice` 積み上げ）と加工利益の釣り合いだけが動き、
 *   「作る > 転売」の幅も改装費の重さも別物になる。**
 *   `k=1.1` での中央一致値は 5.54 で、**5.5 に丸めた**（ずれ −0.7%）。
 */
const PROFIT_SCALE = 5.5
