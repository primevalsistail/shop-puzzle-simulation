 ...
 *
 *     craftMultiplier = 1.30 + 0.25 × min(所要分 / 240, 2) + 0.15 × 贅沢さの順位
 *
 *   ⚠ **2026-09-07（#25 の後）に尺度を縮めた。**所要時間の上限を **1440分 → 480分** にし、
 *   時間項の分母を 480 → 240 にして**上限で頭打ちにならない**ようにした。
 *   理由: 起きている時間は **1080分/日** しかなく、1440分の加工は**何時に始めても日をまたぐ**ため
 *   着手できなかった（#25 Q3 = B）。**チーズ・干し肉・ぶどうの酒が作れなくなっていた。**
 *
...
export const ALL_RECIPES: readonly RecipeDef[] = [
  ...
  //   1.30 + 0.25 × min(所要分 / 240, 2) + 0.15 × 贅沢さの順位（10品とも `日用` なので順位0）
  {
