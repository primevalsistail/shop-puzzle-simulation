 ...
 *
 *     売値（tier2以上） = 材料費 ＋ 加工利益
 *     加工利益          = 5.5 × 升数^1.1 × 格の倍率
 *
 *   ⚠ **加工利益に `durationMinutes` も `outputQuantity` も入らない**（2026-09-15 に外した。
 *   計画 `price-model-rework.md`）。**このファイルの所要時間と出力数は、
 *   「その日に何個作れるか」だけを決める。**値段は動かさない。
 *   **材料費の側だけが今までどおり `outputQuantity` で割られる**（1個あたりの材料費）。
 *
 *   **INV-6（作った品は材料より高い）は、加工利益が常に正であることから定義的に成立する。**
 *   根拠は derive.ts の `craftProfit`（`升数 >= 1` なので常に正）。
 *
 ...
 *
 *   実際の所要時間は手際で短くなる（craft.ts の `craftMinutes`）:
 *     所要 = durationMinutes ÷ 手際の累計倍率（初期 1.0 → 上限 5.0）
 *   ⚠ **品の深さは見ない**（2026-09-15 に S−D 方式を外した）。
