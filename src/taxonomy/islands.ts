 ...
 *   向く土地ごとの品数は 温暖28 ／ 寒い32 ／ 暑い22 ／ 実り14 と 2.3倍 開いている。
 *   4行とも同じ倍率にすると、**品数の多い土地に向く品ばかりが得**になる。
 *   **`品数 ×（倍率−1）` が4行でそろう**ように置き、島ごとの引きの強さを合わせた。
 *   **品数で重みを付けた平均は 1.40。**⚠ **1.30 から上げた**（段4 の置き直し 2026-09-15）——
 *   **層2 の優先順位「島の需要 ＞ 配置ボーナス」を立てるため**（`stage4-rebalance-160-200.md` 受入条件2）。
 *   1.30 のままだと、上等の品を6つ並べた盤面で**配置のほうが勝っていた**
 *   （需要の差 ×1.228 ＜ 配置の差 ×1.248）。**`layer2-priority` のテストが常時見ている。**
...
export const DEMAND_TABLE: readonly DemandRow[] = [
  ...
  { suitedLand: '暑い土地',   island: 'リナツィア', multiplier: 1.44 },
  { suitedLand: '温暖な土地', island: 'ハルヴェラ', multiplier: 1.34 },
  { suitedLand: '実りの土地', island: 'ノアキータ', multiplier: 1.69 },
] as const
