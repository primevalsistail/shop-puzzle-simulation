 ...
 * ⚠ **この字は `localStorage` のキーそのもの**（`CONFIRM_OFF_KEY` に名前の並びで書く）。
 *   **替えると、その行為の「出さない」設定が既定（出す）に戻る。**
 *   #79 で `マイセット` → `型` に替えたとき、一度そうなっている（2026-09-15。PO 了承済み）。
 */
