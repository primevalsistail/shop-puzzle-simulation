...
export class FloorRenderer {
   ...
   *
   * ⚠ **覆う方式にしないこと。**盤面は最大 13×10 ＝ **x342〜1473** まで伸び、
   *   **キャラ帯の左端（`STRIP_L` ＝ 1470）を 3px はみ出す。**覆うとその帯だけ残る。
   *   ⚠ **はみ出す相手は `PLACE_R` ではない**（いま 1629）。
   *   **#58 で場所の領域をキャラ帯まで広げる前は `PLACE_R` ＝ 976 で、6px はみ出していた。**
   *   **その頃の数がここに残っていた**（#117 で直した）。
   *
   ...
   *
   * ⚠ **盤面をくり抜くこと。**上端の帯（y 0〜84）は**盤面の1行目（y 12〜99）に食い込む**ので、
