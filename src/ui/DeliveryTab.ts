 ...
 *
 * ⚠ **手持ちの列も絵に無い。**代わりに**納められないときだけ、納品ボタンの字が `手持ち/必要` に変わる**
 *   —— 商人タブの「買えないときだけボタンの字が理由に変わる」（PO 指示 2026-09-13）と同じ作り。
 *   **ここを消すと、あと何個で納められるのかが画面から消える。**
 *
...
export class DeliveryTab {
  ...
  private buildRow(row: DeliveryRowView, y: number, objs: Phaser.GameObjects.GameObject[]): void {
    ...
    objs.push(
      ...
      this.scene.add.text(DELIVERY_QTY_R, y, String(order.quantity), {
        fontSize: `${TAB_ROW_SUB_FONT_PX}px`, color: '#ffffff',
      }).setOrigin(1, 0.5),
    ...
    // ── 納品 ── ⚠ **納められないときは押せない。**字は `手持ち/必要` に変わる
    ...
    objs.push(btn, this.scene.add.text(
      DELIVERY_BTN_L + DELIVERY_BTN_W / 2, y,
      ready ? DELIVERY_BTN_LABEL : deliveryShortLabel(held, order.quantity), {
    ...
    discard.on('pointerdown', () => {
      this.onDiscard(order.id)
      this.refresh()
    })
