 ...
 *   救済の品は**買値0・売値5**だが、**1つの品は棚に1区画までしか出せない**
 *   （`PlacementManager.isDisplayed`）ので、**盤面を埋めることはできない。**
 *   天井は **1升 × 5.4個/日 × 5レン ＝ 27レン/日**（客は1区画あたり 5.4個/日 買う）。
 *   **加工の稼ぎを超えないこと**を、`shallow` ／ `deep` と同じ条件で回して比べる。
 *
...
export const rescueOnly: Policy = {
  name: '救済の品だけ',
  // ⚠ **個数は `stockTarget` を素通し。**買える数は在庫の空きと、棚の1区画が決める
